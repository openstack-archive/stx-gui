...
class AddInterface(forms.SelfHandlingForm):
    ...
    iftype = forms.ChoiceField(
        ...
        widget=forms.Select(
            attrs={
                ...
                'data-ifclass-none': 'Interface Type',
                'data-ifclass-platform': 'Interface Type',
                'data-ifclass-data': 'Interface Type',
                'data-ifclass-pci-sriov': 'Interface Type',
                'data-ifclass-pci-passthrough': 'Interface Type',
    ...
    imtu = forms.IntegerField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'switched',
            'data-switch-on': 'ifclass',
            'data-ifclass-none': _('MTU'),
            'data-ifclass-platform': _('MTU'),
            'data-ifclass-data': _('MTU'),
            'data-ifclass-pci-sriov': _('MTU'),
            'data-ifclass-pci-passthrough': _('MTU')}))
    ...
    def clean(self):
        ...
        """
        iftype = cleaned_data.get('iftype', 'none')
        uses = cleaned_data.get('uses', [])
        if iftype == 'ae':
            for interface_uuid in uses:
                interface = sysinv.host_interface_get(self.request,
                                                          interface_uuid)
                if interface.ifclass != 'none':
                    raise forms.ValidationError(_(
                        "You cannot create an aggregated ethernet "
                        "interface using an interface that "
                        "is not set to 'none'."))
        """
...
class UpdateInterface(AddInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # updating an interface interface class to 'data-external'
        # if (sdn_enabled and sdn_l3_mode):
        #    choices_list.append('data-external')
        ...
        try:
            ...
            if not data['ifclass'] or data['ifclass'] == 'none':
                ...
                if current_interface.ifclass == 'data':
                    ...
                    #       [str(current_interface.ifclass).split(",")]):
                    #    data['providernetworks'] = 'none'
            ...
            # port_list = sysinv.host_port_list(request, host_uuid)
            # for p in port_list:
            # if p.uuid in ports:
            #        pdata = { 'interface_uuid' : interface.uuid }
            #        sysinv.host_port_update(request, p.uuid, **pdata)
