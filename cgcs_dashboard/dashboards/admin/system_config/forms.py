...
class UpdateiStoragePools(forms.SelfHandlingForm):
    ...
    cinder_pool_gib = forms.IntegerField(
        label=_("Cinder Volumes Pool (GiB)"),
        required=True,
        help_text=_("Storage space allocated to cinder volumes in gibibytes."),
        min_value=0,
        widget=forms.NumberInput(attrs=js_attrs))

    glance_pool_gib = forms.IntegerField(
        label=_("Glance Image Pool (GiB)"),
        required=True,
        help_text=_("Storage space allocated to glance images in gibibytes."),
        min_value=0,
        widget=forms.NumberInput(attrs=js_attrs))

    ephemeral_pool_gib = forms.IntegerField(
        label=_("Ephemeral Storage Pool(Gib)"),
        required=True,
        help_text=_("Storage space allocated to nova ephemeral instance disks "
                    "in gibibytes."),
        min_value=0,
        widget=forms.NumberInput(attrs=js_attrs))

    object_pool_gib = forms.IntegerField(
        label=_("Object Storage Pool(Gib)"),
        required=True,
        help_text=_("Storage space allocated to objects in gibibytes."),
        min_value=0,
        widget=forms.NumberInput(attrs=js_attrs))
