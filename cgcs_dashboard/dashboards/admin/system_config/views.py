...
class UpdateiStoragePoolsView(forms.ModalFormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if storage_list:
            if hasattr(storage_list[0], 'uuid'):
                uuid = storage_list[0].uuid
        ...
        ctxt['uuid'] = uuid
        ctxt['cluster_total'] = storage_list[0].ceph_total_space_gib

        ctxt['configured_quota'] = 0
        # check before adding each value in case it is None
        if storage_list[0].cinder_pool_gib:
            ctxt['configured_quota'] += storage_list[0].cinder_pool_gib
        if storage_list[0].glance_pool_gib:
            ctxt['configured_quota'] += storage_list[0].glance_pool_gib
        if storage_list[0].ephemeral_pool_gib:
            ctxt['configured_quota'] += storage_list[0].ephemeral_pool_gib
        if storage_list[0].object_pool_gib:
            ctxt['configured_quota'] += storage_list[0].object_pool_gib

        return ctxt

    def get_initial(self):

        storage_form_data = {'uuid': ' ',
        ...
        try:
            ...
            if storage_list:
                if storage_list[0]:
                    ...
                    storage_attrs = storage_list[0]._attrs

                    if 'uuid' in storage_attrs:
                        storage_form_data['uuid'] = storage_list[0].uuid

                    if 'cinder_pool_gib' in storage_attrs:
                        storage_form_data['cinder_pool_gib'] = storage_list[
                            0].cinder_pool_gib

                    if 'glance_pool_gib' in storage_attrs:
                        storage_form_data['glance_pool_gib'] = storage_list[
                            0].glance_pool_gib

                    if 'ephemeral_pool_gib' in storage_attrs:
                        storage_form_data['ephemeral_pool_gib'] = storage_list[
                            0].ephemeral_pool_gib

                    if 'object_pool_gib' in storage_attrs:
                        storage_form_data['object_pool_gib'] = storage_list[
                            0].object_pool_gib
